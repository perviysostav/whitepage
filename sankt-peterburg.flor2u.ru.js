...
var name = $xcntJQuery("h1.detail__title:first").text();
...
var category = ''
var Path = 'div.breadcrumbs:first > div.breadcrumbs__item  > a.bread-crumbs_link'
if ($xcntJQuery(Path).length>0){
 ...
 var catLink = $xcntJQuery(Path).eq($xcntJQuery(Path).length-1).attr('href');
...
var price = $xcntJQuery('div.detail__block-right-top:first > input[data-item-price]').val();
if ((typeof price === 'string')&&(price != ''))
{
   price = $xcntJQuery.trim(price.replace(/\s/ig,'').replace(/₽\.?/ig,'').replace(/&nbsp;/ig,'').replace(/\n/ig, '').replace(/\s+/ig, ' '));
